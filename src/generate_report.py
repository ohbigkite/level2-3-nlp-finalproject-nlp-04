...
def generate_report(
    llm_conversation, llm_total, all_conversation_history_lst, job_nm, main_job_ability, question_prompt, total_prompt
):
    """
    면접 리포트를 생성하는 함수입니다.
...
def kpi_radar_chart(kpi_report):
    ...
    layout = go.Layout(
        polar=dict(radialaxis=dict(visible=False, range=[0, 100], showticklabels=True)),
        paper_bgcolor="white",
        plot_bgcolor="#2D5AF0",
        width=400,
        height=300,
